...
function shellGuard(input: ShellInput): ToolGuardResult {
    ...
    if (denied) {
        return {result: 'denied', reason: i18nInstance.t('agent.tools.shell.guard.danger', {command: denied})};
    }
    const warned = checkRules(rules.warning, command);
    if (warned) {
        return askPermissionGuard(i18nInstance.t('agent.tools.shell.guard.warn', {command: warned}));
    }
    if (agentMode !== 'agent') {
        return askPermissionGuard(i18nInstance.t('agent.tools.shell.guard.mode', {command: command}));
    }
...
async function runCommand(input: ShellInput): Promise<string> {
    ...
    try {
        ...
        return output ? output.slice(0, trunkcateThreshold) : i18nInstance.t('agent.tools.shell.empty');
    } catch (error: any) {
        return error?.killed && error?.signal === 'SIGTERM' ? i18nInstance.t('agent.tools.shell.timeout', {timeout})
            : i18nInstance.t('agent.tools.shell.error', {message: error?.message || ''});
    }
}
