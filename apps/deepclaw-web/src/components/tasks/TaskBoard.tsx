    ...
    <div className="h-full flex flex-col bg-gray-100">
      <div className="px-4 sm:px-6 py-4 bg-white border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-600">
          <span className="flex items-center gap-1"><CheckCircle2 size={16} className="text-green-500" />{projects.filter(p => !!p.closedAt).length} {t('pages.projects.status.done')}</span>
          <span className="flex items-center gap-1"><Clock size={16} className="text-yellow-500" />{projects.filter(p => !p.closedAt).length} {t('pages.projects.status.ongoing')}</span>
        </div>
      </div>
