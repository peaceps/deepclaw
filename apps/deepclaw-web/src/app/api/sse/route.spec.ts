...
const mocks = vi.hoisted(() => ({
    newSSEEndpoint: vi.fn<(browserId: string) => Response>(),
}));

vi.mock('../sse-endpoint', () => ({newSSEEndpoint: mocks.newSSEEndpoint}));

let stream: Response;

function get(query: string): Promise<Response> {
    return GET(new Request(`http://localhost:3000/api/sse${query}`));
}

beforeEach(() => {
    vi.clearAllMocks();
    stream = new Response('stream');
    mocks.newSSEEndpoint.mockReturnValue(stream);
});

describe('GET', () => {

    test('answers with the stream of the given browser', async () => {
        expect(await get('?browserId=b1')).toBe(stream);
        expect(mocks.newSSEEndpoint).toHaveBeenCalledWith('b1');
    });
    ...
    test('answers 404 with an empty body when the browser id is missing', async () => {
        ...
        expect(mocks.newSSEEndpoint).not.toHaveBeenCalled();
    });
    ...
    test('takes an empty browser id as a named browser', async () => {
        ...
        expect(mocks.newSSEEndpoint).toHaveBeenCalledWith('');
    });
    ...
    test('decodes an escaped browser id', async () => {
        ...
        expect(mocks.newSSEEndpoint).toHaveBeenCalledWith('b 1');
    });
});
